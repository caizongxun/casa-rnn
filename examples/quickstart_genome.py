...
BATCH, SEQ, RAW_FEAT = 32, 60, 5  # raw: open, high, low, close, volume
TOTAL_STEPS  = 400
SWITCH_STEPS = [200]
TRANSITION   = 30
...
def make_batch(step):
    ...
    y = x[:, :, 3:4].roll(-1, dims=1) * sign  # predict next close
...
# Print what features the genome learned
print("\n=== Feature Genome Report ===")
report = model.get_feature_report()
print("Top feature interactions learned:")
for i, j, w in report["top_interactions"]:
    print(f"  feat[{i}] x feat[{j}]  weight={w:.4f}")
